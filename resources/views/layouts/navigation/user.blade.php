<div x-data="{ open: false }">
        ...
        <div>
            <div class="flex items-center gap-3 px-6 h-24">
                <div class="flex items-center justify-center overflow-hidden rounded-lg">
                    <img src="{{ asset('images/logo2.png') }}" alt="PerformPT Logo" class="w-10 h-10 object-contain">
                </div>
                <div>
                    <h1 class="text-white font-bold text-lg leading-none tracking-tight">PerformPT</h1>
                    <p class="text-gray-400 text-[10px] mt-1 font-medium uppercase tracking-wider">Employee Portal</p>
                </div>
            </div>

            <div class="px-4 py-4">
                <p class="text-gray-500 text-[10px] uppercase tracking-[0.2em] font-bold px-4 mb-4">Main Menu</p>
                
                <div class="space-y-2">
                    <a href="{{ route('user.dashboard') }}" 
                        class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('user.dashboard') ? 'bg-[#10b981] text-white shadow-lg shadow-emerald-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
                        <i class="fas fa-chart-pie w-5 text-center"></i>
                        <span class="text-sm font-semibold">My Performance</span>
                    </a>

                    <a href="{{ route('user.trend') }}" 
                        class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('user.trend') ? 'bg-[#10b981] text-white shadow-lg shadow-emerald-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
                        <i class="fas fa-chart-line w-5 text-center"></i>
                        <span class="text-sm font-semibold">Performance Trends</span>
                    </a>

                    <a href="{{ route('user.feedback') }}" 
                        class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('user.feedback') ? 'bg-[#10b981] text-white shadow-lg shadow-emerald-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
        ...
        <div class="p-4 bg-black/10 border-t border-white/5">
            <div class="flex items-center gap-3 px-2 mb-4">
                ...
                <div class="flex-1 overflow-hidden text-left">
                    ...
                    <p class="text-gray-500 text-[9px] font-bold uppercase mt-0.5">Employee</p>
                </div>
            </div>
    ...
    <div class="lg:hidden fixed top-0 left-0 w-full bg-[#1e293b] border-b border-white/10 z-[60] h-16 flex items-center justify-between px-4">
        <div class="flex items-center gap-3">
            <img src="{{ asset('images/logo2.png') }}" alt="PerformPT Logo" class="w-8 h-8 object-contain">
            <span class="font-bold text-white tracking-tight">PerformPT</span>
        </div>
        
        <button @click="open = !open" class="p-2 focus:outline-none" style="color: white !important;">
        ...
        <div x-show="open"
            ...
            <div class="flex flex-col h-full">
                <div class="px-6 h-16 flex items-center border-b border-white/5 bg-black/10">
                    <span class="text-white font-bold uppercase tracking-widest text-xs">Employee Menu</span>
                </div>

                <div class="flex-1 overflow-y-auto px-4 py-6 space-y-2">
                    <a href="{{ route('user.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('user.dashboard') ? 'bg-[#10b981] text-white' : 'text-gray-400' }}">
                        <i class="fas fa-chart-pie w-5 text-center"></i>
                        <span class="text-sm font-semibold">My Performance</span>
                    </a>
                    
                    <a href="{{ route('user.trend') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('user.trend') ? 'bg-[#10b981] text-white' : 'text-gray-400' }}">
                        <i class="fas fa-chart-line w-5 text-center"></i>
                        <span class="text-sm font-semibold">Performance Trends</span>
                    </a>

                    <a href="{{ route('user.feedback') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('user.feedback') ? 'bg-[#10b981] text-white' : 'text-gray-400' }}">
                        <i class="fas fa-comment-dots w-5 text-center"></i>
                        <span class="text-sm font-semibold">Feedback & Remarks</span>
                    </a>
                </div>
                ...
                <div class="p-4 bg-black/10 border-t border-white/5">
                    <div class="flex items-center gap-3 px-2 mb-4">
                        ...
                        <div class="flex-1 overflow-hidden text-left">
                            ...
                            <p class="text-gray-500 text-[9px] font-bold uppercase">Employee</p>
                        </div>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full py-2.5 rounded-lg bg-white/5 text-gray-300 text-xs font-bold border border-white/5 uppercase tracking-wider">
                            Sign out
                        </button>
                    </form>
                </div>
